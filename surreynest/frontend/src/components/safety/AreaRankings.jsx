    ...
    return (
        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.06)] border border-slate-100">
            ...
            <div className="space-y-2">
                {areas.map((area, i) => (
                    <div key={area.postcode_sector || i} className={`flex items-center justify-between px-3.5 py-3 rounded-xl text-xs transition-colors ${area.highlight ? 'bg-primary/10/80 border-2 border-primary/20 font-bold shadow-sm' : 'bg-slate-50/60 hover:bg-slate-50'
                        }`}>
                        <span className="flex items-center gap-2">
                            <span className="text-base">{emoji[i] || `${i + 1}.`}</span>
                            <span className={`font-bold ${area.highlight ? 'text-primary/90' : 'text-slate-700'}`}>
                                {area.postcode_sector}
                                {area.highlight && <span className="ml-1.5 px-1.5 py-0.5 rounded bg-primary-200/50 text-primary/90 text-[10px] uppercase tracking-wider">(you)</span>}
                            </span>
                        </span>
