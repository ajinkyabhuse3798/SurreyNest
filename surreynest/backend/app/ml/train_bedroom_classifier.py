...
def train_and_predict():
    ...
    try:
        ...
        if len(train_df) < 50:
            logger.error("Not enough training data. Need at least 50 scraped properties with actual_bedrooms.")
            return

        X_train_full = encode_features(train_df)
        y_train_full = train_df['actual_bedrooms'].astype(int)
        
        # Train/Test Split just for metrics logging
        ...
        logger.info(f"Sub-Model Validation Accuracy: {acc:.2f}")
        ...
        if not target_df.empty:
            ...
            update_data = [
                {"uprn": uprn, "predicted_beds": int(pred), "now": datetime.now(timezone.utc)}
            ...
            logger.info(f"Successfully backfilled actual_bedrooms for {len(target_df)} properties.")
