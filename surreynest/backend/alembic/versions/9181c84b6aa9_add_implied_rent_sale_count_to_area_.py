...
def upgrade() -> None:
    op.add_column(
        "area_values", sa.Column("implied_weekly_rent", sa.Float(), nullable=True)
    )
    op.add_column("area_values", sa.Column("sale_count", sa.Float(), nullable=True))
    op.alter_column(
        "area_values", "median_sale_price", existing_type=sa.Float(), nullable=True
    )


def downgrade() -> None:
    op.alter_column(
        "area_values", "median_sale_price", existing_type=sa.Float(), nullable=False
    )
