"""add is_university flag

Revision ID: 11c4ed46d4db
Revises: 40b58f8feae3
Create Date: 2026-03-09 22:48:21.703940
...
revision: str = '11c4ed46d4db'
down_revision: Union[str, None] = '40b58f8feae3'
...
def upgrade() -> None:
    ...
    op.add_column('properties', sa.Column('is_university', sa.Boolean(), server_default='false', nullable=False))
    op.drop_index('ix_properties_geom', table_name='properties', postgresql_using='gist', postgresql_where='((lat IS NOT NULL) AND (lng IS NOT NULL))')
    ...
    op.drop_index('ix_voa_rent_bands_local_authority_code', table_name='voa_rent_bands')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_index('ix_voa_rent_bands_local_authority_code', 'voa_rent_bands', ['local_authority_code'], unique=False)
    op.create_index('ix_rent_history_sector', 'rent_history', ['postcode_sector'], unique=False)
    op.add_column('properties', sa.Column('is_university_managed', sa.BOOLEAN(), server_default=sa.text('false'), autoincrement=False, nullable=False))
    op.create_index('ix_properties_is_university_managed', 'properties', ['is_university_managed'], unique=False)
    op.create_index('ix_properties_geom', 'properties', [sa.text('st_point(lng, lat)')], unique=False, postgresql_using='gist', postgresql_where='((lat IS NOT NULL) AND (lng IS NOT NULL))')
    op.drop_column('properties', 'is_university')
