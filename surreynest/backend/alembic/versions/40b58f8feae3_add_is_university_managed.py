...
def upgrade() -> None:
    # Skip dropping PostGIS tiger geocoder objects. On managed Postgres they may
    # be absent, and on PostGIS-backed databases they are extension-owned.
    op.add_column(
        "properties", sa.Column("is_university_managed", sa.Boolean(), nullable=False)
    )
    op.drop_index(
        "ix_properties_geom",
        ...
        postgresql_where="((lat IS NOT NULL) AND (lng IS NOT NULL))",
    )
    op.create_index(
        op.f("ix_properties_is_university_managed"),
        "properties",
        ["is_university_managed"],
        unique=False,
    )
    op.drop_index("ix_rent_history_sector", table_name="rent_history", if_exists=True)
    op.drop_index(
...
def downgrade() -> None:
    ...
    op.create_index(
        "ix_rent_history_sector", "rent_history", ["postcode_sector"], unique=False
    )
    op.drop_index(op.f("ix_properties_is_university_managed"), table_name="properties")
    op.create_index(
        "ix_properties_geom",
        "properties",
        [sa.text("st_point(lng, lat)")],
        unique=False,
        postgresql_using="gist",
        postgresql_where="((lat IS NOT NULL) AND (lng IS NOT NULL))",
    )
